...
def gaussian_mixture(batchsize, num_leaves, sel):
    ...
    for batch in range(batchsize):
        for zi in range(2 // 2):
            ...
            z[batch, zi*2:zi*2+2] = np.array([x[batch, zi], y[batch, zi]]).reshape((2,))
...
with tf.variable_scope('Discriminator'):
    ...
    d_fake = discriminator(encoder_outputZ,tf.one_hot(tf.argmax(encoder_outputYlogits, dimension = 1), depth = n_leaves), reuse=True)
